...
class SRSService:
    ...
    def _generate_simulation_paragraph(self, physician: str, physicist: str,
        ...
        text += f"where a rigid body fusion was performed. "
    ...
    def _generate_planning_paragraph(self, lesions: List) -> str:
        """Generate treatment planning paragraph with metrics."""
        if len(lesions) == 1:
            # Single lesion case
            lesion = lesions[0]
            ...
            # Plain text metrics
            text += f"• Prescription Dose: {self._format_number(lesion.dose)} Gy in {lesion_fraction_text}\n"
            text += f"• Target Volume: {self._format_number(lesion.volume)} cc\n"
            text += f"• Location: {lesion.site}\n"
            text += f"• Prescription Isodose: {self._format_number(lesion.prescription_isodose)}%\n"
            text += f"• PTV Coverage: {self._format_number(lesion.ptv_coverage)}%\n"
            text += f"• Conformity Index: {self._format_number(lesion.conformity_index, 2)}\n"
            text += f"• Gradient Index: {self._format_number(lesion.gradient_index, 2)}\n"
            text += f"• Maximum Dose: {lesion.max_dose}%"
        else:
            ...
            for i, lesion in enumerate(lesions):
                ...
                text += f"• Dose: {self._format_number(lesion.dose)} Gy in {lesion.fractions} fraction{'s' if lesion.fractions > 1 else ''}\n"
                text += f"• Prescription Isodose: {self._format_number(lesion.prescription_isodose)}%\n"
                text += f"• PTV Coverage: {self._format_number(lesion.ptv_coverage)}%\n"
                text += f"• Conformity Index: {self._format_number(lesion.conformity_index, 2)}\n"
                text += f"• Gradient Index: {self._format_number(lesion.gradient_index, 2)}\n"
            ...
            if len(fractions_set) == 1 and len(treatment_types_set) == 1:
                ...
                if lesions[0].treatment_type == "SRS":
                    ...
                    fraction_text = f"{lesions[0].fractions} fractions"
